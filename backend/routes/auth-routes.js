...
router.post("/login", async (req, res) => {
  try {
    ...
    if (validPassword && !vaildEmail) {
      let tokens = jwtTokens(
        users.rows[0].user_email,
        users.rows[0].user_password,
        users.rows[0].user_name,
        users.rows[0].user_firstname,
        users.rows[0].user_lastname,
        users.rows[0].user_mobile,
        users.rows[0].user_birthday
      );
